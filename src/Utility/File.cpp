...
namespace Utility
{
	...
	void File::set_path(const std::string &root, const std::vector<std::string> &uri_paths) {
		//"www/wordpress/index.html/" //TODO modified path?
		_path += root;
	...
	bool File::is_directory(void) {
		struct stat s;
		if (stat(_path.c_str(), &s) == 0) // what happens if it doesn't exist? > should I check this first before everythin else in serve_file
		{
