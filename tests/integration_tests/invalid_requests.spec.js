...
describe("Sending MALICIOUS requests", () => {
    it("Non-supported method results in Bad Request", async () => {
        expect.assertions(1);
        try {
    ...
    it("Long method results in Not Implemented", async () => {
        expect.assertions(1);
        try {
            const response = await axios
                .create({
                ...
                .request("http://localhost:80/sth");
        } catch (error) {
            expect(error.response.status).toEqual(501);
        }
    });
});
...
describe("Sending LONG requests to the webserver", () => {
    it("Too long uri results in URITooLong", async () => {
        ...
        try {
            ...
            expect(error.response.status).toEqual(414);
        }
    });
});
